 ...
 * (Motor portado do projeto Mythical Gym; ver a skill scroll-video-animation.)
 *
 ...
 * - As BORDAS dissolvem na viagem, não no pin: a cortina cor de paper segue
 *   a posição da secção no viewport (`edgeFade`) — funde da luz da página
 *   enquanto a secção desliza para dentro de cena e para essa luz enquanto
 *   desliza para fora. Durante todo o scrub pinado o filme está aceso; a
 *   página só "recupera" o plano onde a secção já está a sair de quadro.
 *   Numa casa clara o dissolve é névoa de manhã, não blackout.
 *
...
interface Props {
  /** Caminho público da sequência, ex. "/media/sequences/descida". */
  base: string;
  frameCount: number;
...
export default function ScrollSequence({
  ...
  useEffect(() => {
    ...
    // não pode acordar tapado) e a sair (rect.bottom a subir para 0), com a
    // cortina totalmente opaca exatamente quando a secção sai de quadro.
    // Opacidade direta no estilo, fora do React: muda a cada tick e um
    // setState aqui seria um re-render por fotograma.
